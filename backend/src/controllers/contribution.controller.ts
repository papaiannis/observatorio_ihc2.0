...
export const createContribution = async (
  ...
  try {
    ...
    if (!userToken) throw new AppError('Token de autenticación no disponible', 401);

    const file = req.file;
    if (!file) throw new AppError('La foto es requerida', 400);
...
export const validateContribution = async (
  ...
  try {
    ...
    if (error) throw new AppError(error.message, 500);
    if (!data) throw new AppError('Contribución no encontrada.', 404);

    if (data.user_id) {
      await sendPushAndCreateNotification(
        data.user_id,
        '¡Tu contribución al proyecto fue validada!',
        `Un especialista ha validado tu contribución en el proyecto como "${species.scientific_name}". ¡Excelente observación!`,
        null,
        data.id,
      );
    }
 ...
 * Solo accesible por Especialistas.
 */
export const appealContribution = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (data.user_id) {
      await sendPushAndCreateNotification(
        ...
        'Tu contribución al proyecto está siendo revisada y debatida por los especialistas del observatorio.',
        null,
        data.id,
      );
    }
